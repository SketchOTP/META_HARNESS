...
| `memory.py` | `project_memory.json`, compact prompt context, ASCII map, pattern inference; optional embedding lines |
| `embedding_retrieval.py` | Optional semantic recall (SQLite vectors; OpenAI or local sentence-transformers) |
...
| Date | Summary | Notes |
|------|---------|--------|
| 2026-04-09 | **Leaner diagnosis prompts + KG query context + optional embeddings** | `[evidence]` adds bounded `format_evidence_for_diagnosis` and caps for test output, logs, git diff, cycle history, file tree; metrics default to compact JSON in diagnosis. `KnowledgeGraph.build_compact_context` adds FTS + `git_recent_paths` **rel:** line when `[memory] kg_use_query_context` (default on). `[embedding]` optional: index completed directive bodies to `.metaharness/memory/embedding_index.sqlite`, append **emb:** lines to compact context (OpenAI or local `sentence-transformers`; not in base deps). `tests/conftest.py` stubs Cursor CLI for CI and re-binds `test_cursor_client` after CLI reloads. New `embedding_retrieval.py`. |
